...
public class LineChartController {
	...
	public void initialize() {
		...
		//TODO Date wäre auch möglich - Date-Time, nach ConnectionCode sortiert?
	...
	public void prepareData(ObservableList<RegularBundleFXML> bundles) {
		...
		for (RegularBundleFXML rb : mainIoT.getBundles()) {
			series1.getData().add(new XYChart.Data<String, String>(dateFormat.format(rb.clientTime), rb.connectionCode.getValue()));
		}
		lineChart.getData().add(series1);
		
		SimpleDateFormat formatter = new SimpleDateFormat("dd.MM.yyyy");
		series1.setName(formatter.format(mainIoT.getBundles().get(1).clientTime)); //TODO: getClientNames()
